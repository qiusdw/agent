...
try:
    # 首先尝试从 langchain_classic.agents 导入（LangChain 1.0+ 版本）
    from langchain_classic.agents import AgentExecutor, LLMSingleActionAgent, AgentOutputParser
except ImportError:
    try:
        # 尝试从 langchain.agents 直接导入（旧版本）
        from langchain.agents import AgentExecutor, LLMSingleActionAgent, AgentOutputParser
    except ImportError:
        raise ImportError(
            "无法导入 AgentExecutor, LLMSingleActionAgent, AgentOutputParser\n"
            "这些类在 LangChain 1.0+ 版本中已移至 langchain-classic 包\n"
            "请运行以下命令安装：pip install langchain-classic"
        )

from langchain_core.prompts import StringPromptTemplate, PromptTemplate
from langchain_community.llms import Tongyi
# 兼容不同版本的 LangChain 导入 LLMChain
try:
...
# 通义千问API密钥（从环境变量读取，避免硬编码）
DASHSCOPE_API_KEY = os.getenv('DASHSCOPE_API_KEY', '').strip()
if not DASHSCOPE_API_KEY:
    raise ValueError("请设置环境变量 DASHSCOPE_API_KEY 或在代码中配置API密钥")
...
class FundRulesDataSource:
    ...
    def answer_question(self, query: str) -> str:
        ...
        if best_score < 0.2 or best_rule is None:
            ...
            # 使用 invoke 或 predict 方法调用 LLM
            try:
                if hasattr(self.llm, 'invoke'):
                    response = self.llm.invoke(prompt)
                    # invoke 可能返回字符串或对象，需要提取内容
                ...
                # 如果所有方法都失败，尝试直接调用（兼容旧版本）
                response_text = self.llm(prompt)
            # 返回格式化后的回答，添加特殊标记让 Agent 识别为最终答案
            # 注意：这里返回的内容会被 Agent 作为 Observation，Prompt 模板会检测并生成 Final Answer
            return f"FINAL_ANSWER: 这个问题超出了知识库范围。\n\n{response_text}"
...
class CustomPromptTemplate(StringPromptTemplate):
    ...
    def format(self, **kwargs) -> str:
        ...
        for action, observation in intermediate_steps:
            thoughts += action.log
            # 检查 Observation 是否包含最终答案标记
            if isinstance(observation, str) and "FINAL_ANSWER:" in observation:
                # 提取最终答案并强制结束
                final_answer = observation.split("FINAL_ANSWER:")[-1].strip()
                # 清理答案，移除多余的空行
                final_answer = re.sub(r'\n\s*\n', '\n', final_answer).strip()
                # 直接生成 Final Answer，不添加 Thought，强制结束
                thoughts += f"\nObservation: {final_answer}\nFinal Answer: {final_answer}"
                has_final_answer = True
                break
            thoughts += f"\nObservation: {observation}\nThought: "
        
        # 如果检测到最终答案，修改模板，强制 LLM 输出 Final Answer
        if has_final_answer:
            kwargs["tools"] = "（无可用工具，请直接输出 Final Answer）"  # 明确告知无工具可用
            # 确保 thoughts 中包含 Final Answer，并且格式正确
            # 如果还没有 Final Answer，从 Observation 中提取
            if "Final Answer:" not in thoughts:
                # 从 Observation 中提取最终答案
                final_answer_match = re.search(r"Observation:\s*(.*?)(?=\n(?:Thought|Final Answer|$))", thoughts, re.DOTALL)
                if final_answer_match:
                    final_answer = final_answer_match.group(1).strip()
                    # 移除开头的 "这个问题超出了知识库范围。" 等前缀
                    final_answer = re.sub(r'^这个问题超出了知识库范围[。\s]*\n*', '', final_answer).strip()
                    # 确保有 Final Answer 格式
                    thoughts = thoughts.rstrip() + f"\nFinal Answer: {final_answer}"
            # 在 thoughts 末尾添加明确的结束指示
            if not thoughts.endswith("\nFinal Answer:"):
                # 确保最后是 Final Answer 格式
                if "Final Answer:" in thoughts:
                    # 如果已经有 Final Answer，确保格式正确
                    pass
                else:
                    # 如果没有，添加
                    final_answer_match = re.search(r"Observation:\s*(.*?)(?=\n(?:Thought|Final Answer|$))", thoughts, re.DOTALL)
        ...
        # 确保 agent_scratchpad 被添加到 kwargs 中
...
class CustomOutputParser(AgentOutputParser):
    def parse(self, llm_output: str) -> Union[AgentAction, AgentFinish]:
        ...
        # 优先检查：如果输出包含"Final Answer:"，直接处理为最终答案
        # 使用更精确的正则表达式提取 Final Answer
        final_answer_match = re.search(r"Final Answer:\s*(.*?)(?=\n(?:Question|Action|Thought|$))", llm_output, re.DOTALL)
        if final_answer_match:
            final_answer = final_answer_match.group(1).strip()
            # 清理可能的换行和多余空格，但保留段落结构
            final_answer = re.sub(r'\n{3,}', '\n\n', final_answer).strip()
            return AgentFinish(
        ...
        # 如果输出直接以 "Final Answer:" 开头（没有其他内容）
        if llm_output.strip().startswith("Final Answer:"):
        ...
        # 检查是否包含 "我已经得到了最终答案" 或类似的明确结束信号
        if "我已经得到了最终答案" in llm_output or "我已经知道答案了" in llm_output:
            # 尝试提取 Final Answer
            ...
            # 如果没有 Final Answer，从 Thought 中提取
            thought_match = re.search(r"Thought:\s*我已经.*?答案[。！]?\s*(.*?)(?=\n|$)", llm_output, re.DOTALL)
            if thought_match:
                return AgentFinish(
                    return_values={"output": thought_match.group(1).strip() if thought_match.group(1) else "我已经可以回答这个问题了。"},
                    log=llm_output,
                )
        
        # 检查是否包含 "Thought:" 和 "现在我知道答案了" 或类似的结束思考
        if "Thought:" in llm_output:
            # 提取最后一个 Thought 的内容
            thoughts = re.findall(r"Thought:\s*(.*?)(?=\n(?:Action|Final Answer|$))", llm_output, re.DOTALL)
            if thoughts:
                last_thought = thoughts[-1].strip().lower()
                # 如果最后的思考表示已经知道答案，尝试提取 Final Answer
                if any(phrase in last_thought for phrase in ["现在我知道答案了", "我可以给出答案", "答案如下", "我知道答案"]):
                    # 尝试提取 Final Answer
                    if "Final Answer:" in llm_output:
                        return AgentFinish(
                    ...
                    # 如果没有 Final Answer，但思考表示知道答案，也结束
                    return AgentFinish(
                        return_values={"output": "根据我的分析，我已经可以回答这个问题了。"},
                        log=llm_output,
                    )
...
def create_fund_qa_agent():
    ...
    # 创建Agent执行器，设置最大迭代次数防止无限循环
    agent_executor = AgentExecutor.from_agent_and_tools(
        agent=agent, tools=tools, verbose=True, max_iterations=5, max_execution_time=60
    )
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # 使用 invoke 方法替代已弃用的 run 方法
            if hasattr(fund_qa_agent, 'invoke'):
                result = fund_qa_agent.invoke({"input": user_input})
                # invoke 返回字典，需要提取 output 字段
