...
try:
    ...
    try:
        ...
        # 如果以上都失败，尝试从 langchain_core 导入（旧版本兼容）
        try:
            from langchain_core.pydantic_v1 import BaseModel, Field
        except ImportError:
            raise ImportError("无法导入 BaseModel 和 Field，请安装 pydantic: pip install pydantic")
...
# Tool 从 langchain_core.tools 导入
try:
    ...
    try:
        ...
        class ReActPromptTemplate(StringPromptTemplate):
            def format(self, **kwargs):
                ...
                prompt_text = f"""你是一个财富管理投顾AI助手。请根据用户问题，决定是否需要调用工具。
...
请按以下格式回答：
- 如果需要调用工具，请输出：Action: [工具名称]
Action Input: [工具输入]
- 如果不需要调用工具或已经得到答案，请直接输出最终答案。

现在请回答："""
                return prompt_text

        prompt = ReActPromptTemplate(input_variables=["input", "intermediate_steps"])
        llm_chain = LLMChain(llm=llm, prompt=prompt)
        tool_names = [tool.name for tool in tools]

        # 修正：正确解析Agent输出，支持工具调用
            ...
            def parse(self, text):
                ...
                if "Action:" in text:
                    ...
                    if action_match:
                        ...
                        # 验证工具名称是否在允许列表中（完全匹配或部分匹配）
                        matched_tool = None
                        for tool_name_in_list in tool_names:
                            if tool_name_in_list in tool_name or tool_name in tool_name_in_list:
                                matched_tool = tool_name_in_list
                                break
        ...
        agent_executor = AgentExecutor.from_agent_and_tools(
            agent=agent, tools=tools, verbose=True  # 改为True以便调试
        )
        # 运行Agent
        user_query = state["user_query"]
