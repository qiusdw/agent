...
def init_agent_service():
    ...
    # MCP 工具配置
    # 注意：stdio 模式不需要 port 参数，port 仅用于 SSE 模式
    # 获取 txt_counter.py 的绝对路径，确保 MCP Server 能正确启动
    txt_counter_path = os.path.join(os.path.dirname(__file__), "txt_counter.py")
    tools = [{
        "mcpServers": {
            "txt-counter": {
                "command": "python",
